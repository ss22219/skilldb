...
### 1.2 商品精准清洗规则
系统自动过滤掉容易混淆的非 Plus 商品（例如 `非Plus`、`不含Plus`、`媲美Plus`、`可升级Plus`、`开plus绑定专用`、`99%开plus` 等）。
商品自动归类为四种核心形态：
1. **成品号**: 独享/共享 Plus 账号、首登号、月卡账号。
2. **代充/直充**: Plus 官方直充、代充链接、充值卡。
3. **兑换码/卡密**: CDK、礼品卡、激活码。
4. **团队/Pro**: ChatGPT Team 团队号、Pro 商业版。
