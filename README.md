...
| **`sk-api-relay-monetization`** | `/sk-api-relay` | **大模型 API 中转站赚钱**：开源 `sub2api` / `Shop2API` / `New-API` 部署演进、海外 VPS 免备案购买、AI 驱动自动部署、联动小铺/发卡网 CDK 售卖、全套运维成本评估与 10% 薄利跑量/高毛利定价模型。 |
...
- **架构与部署演进**：新手初级阶段单节点部署 `sub2api` / `Shop2API` 挂载 Plus 订阅号池；业务扩容阶段多节点部署并由 `New-API` 统一路由网关聚合做负载均衡。
